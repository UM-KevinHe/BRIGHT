...
<table>
    ...
    <tr>
        ...
        <td><a href="https://um-kevinhe.github.io/BRIGHT/articles/BRIGHTs.html">tutorial</a></td>
    </tr>
      <tr>
        <td>BRIGHTi</td>
        <td>
        ...
        <td><a href="https://um-kevinhe.github.io/BRIGHT/articles/BRIGHTi.html">tutorial</a></td>
    </tr>
</table>
...
## 4.2 BRIGHTs model fitting and visualization
...
# 5 BRIGHTs with summary-level target data

## 5.1 Identity link function
...
#### 5.1.1.1 LD reference
BRIGHTs need to account for Linkage Disequilibrium (LD) via a reference panel (1000 genomes project as default).
...
#### 5.1.1.2 Target GWAS + Prior GWAS

We first load the GWAS summary statistics for target data. 
...
Then we load the GWAS summary statistics for prior data.
```{r}
...
We require the GWAS summary statistics from both the target minority data and prior data to contain 7 columns ordered exactly as above: the "CHR" column is the chromosome name and must be from 1 to 22; the "BP" column is the corresponding SNPs position in base pair; "A1" column is the effect allele and "A2" column is the alternative allele; "Sign" is the marginal association direction for each SNPs (the sign of the beta coefficients); "N" column is the sample size used for each SNPs from GWAS study; "P" column is the p-value of the t-test from GWAS.

#### 5.1.1.3 Target marginal correlation + Prior marginal correlation

We first load the SNP-wise (marginal) correlation summary statistics for target data. 
...
Then we load the SNP-wise (marginal) correlation summary statistics for prior data. 
```{r}
...
head(Pss)
```
We require the correlation summary statistics from both the target minority data and the prior data to contain 5 columns ordered exactly as above: the "CHR" column is the chromosome name and must be from 1 to 22; the "BP" column is the corresponding SNPs position in base pair; "A1" column is the effect allele and "A2" column is the alternative allele; the "Corr" column contains the marginal genotype-trait correlation.

#### 5.1.1.4 Target GWAS + Prior PRS coefficients

First target GWAS data can be loaded exactly as mentioned in section 5.1.1.2. Then we load the PRS coefficients from a joint model for prior data.
```{r}
...
#### 5.1.1.5 Target GWAS + Prior marginal correlation
Target GWAS data can be loaded exactly as mentioned in section 5.1.1.2 and prior SNP-wise (marginal) correlation data can be loaded exactly as mentioned in section 5.1.1.3

#### 5.1.1.6 Target marginal correlation + Prior GWAS
Target SNP-wise (marginal) correlation data can be loaded exactly as mentioned in section 5.1.1.3 and prior GWAS data can be loaded exactly as mentioned in section 5.1.1.2

#### 5.1.1.7 Target marginal correlation + Prior PRS coefficients
Target SNP-wise (marginal) correlation data can be loaded exactly as mentioned in section 5.1.1.3 and prior PRS coefficients data (from joint model) can be loaded exactly as mentioned in section 5.1.1.4

#### 5.1.1.2 Target minority input
...
### 5.1.2 Preprocessing and model fitting
...
### 5.1.3 Hyperparameter fine-tuning
...
#### 5.1.3.2 Hyperparameter fine-tuning with individual-level validation data
...
#### 5.1.3.3 Hyperparameter fine-tuning with summary-level validation data
...
### 5.1.4 Model testing based on individual-level testing data
...
# 6 BRIGHTi with individual-level target data
...
## 6.2 Hyperparameter fine-tuning with cross-validation
