...
QC <- function(ss,ind,KG,lab){
  ...
  writeLines(strwrap(paste(sum(pos)+sum(pos_flp)," SNPs passed filtering for ",lab," data",sep="")))
...
BRIGHTs=function(data,m=NA,group=NA,penalty=1,tau=0,eta=0,lambda=NA,nlambda=100,lambda.min=0.001,alpha=1, gamma=0,eps=0.0000001,max_iter=1000000, dfmax=5000, gmax=5000, user=T){
  if(is.null(data$Tss$IProd)){
  ...
  for(eta in eta_vec){
    ...
    out[[as.character(eta)]]=rst_BRIGHTs$Beta
  }
...
#' for bi-level selection.
#' @return A list of preprocessed XtY and reference genotype matrix.
PreprocessI=function(Tpheno, Tgeno, Pss=NA, Pind=NA, Pref=NA){
  
  # read in target genotype
  writeLines(strwrap("Using individual-level genotype matrix"))
  KG=read.table(paste(Tgeno,".bim",sep=""))
  KG[,2]=paste(KG[,1],KG[,4],KG[,5],KG[,6],sep="_")
  LD_dir=paste(Tgeno,".bed",sep="")
  
  writeLines(strwrap(paste(length(Pind)," source of prior information identified",sep="")))
  
  Tgeno_ID=read.table(paste(Tgeno,".fam",sep=""))
  Tgeno_ID=paste(Tgeno_ID[,1],Tgeno_ID[,2],sep="_")
  Tpheno_ID=paste(Tpheno[,1],Tpheno[,2],sep="_")
  
  if(length(Tgeno_ID)!=length(Tpheno_ID)){
    stop("numbers of samples are different between phenotype files and genotype files")
  }else{
    Tpheno=Tpheno[match(Tpheno_ID,Tgeno_ID),]
  }
...
#' }
BRIGHTi=function(data,m=NA,group=NA,Beta_prior,penalty="grLasso",family="gaussian",eta,lambda,lambda.min,tau=1/3,alpha=1, gamma=ifelse(penalty == "grSCAD", 4, 3),eps=0.0000001,max_iter=1000000, dfmax=5000, gmax=5000){
  KG=data$KG
  ...
  rst_BRIGHTi=grpreg(X=gene_bed,y=Y,group=group,penalty = penalty,family = family,lambda = lambda,alpha=alpha,eps=eps,max.iter = max_iter,dfmax = dfmax,gmax = gmax,tau = tau,gamma = gamma)
