...
BRIGHTs=function(data,m=NA,group=NA,penalty=1,tau=0,eta=c(0,exp(seq(log(0.1),log(10),length.out=20))),lambda=NA,nlambda=100,lambda.min=0.001,alpha=1, gamma=0,eps=0.0000001,max_iter=1000000, dfmax=5000, gmax=5000, user=T){
  ...
  gene_bed = BEDMatrix::BEDMatrix(paste(data$Tref,".bed",sep=""))
  ...
  XtY <- t(t(data$Tss$Corr))
  
  #LDB=read.table(paste("inst/data/Berisa.",data$TLDblocks,".bed",sep=""),header=T)
  if(data$TLDblocks=="EUR.hg19"){
    LDB=EUR.hg19
  }else if(data$TLDblocks=="EUR.hg38"){
    LDB=EUR.hg38
  }else if(data$TLDblocks=="AFR.hg19"){
    LDB=AFR.hg19
  }else if(data$TLDblocks=="AFR.hg38"){
    LDB=AFR.hg38
  }else if(data$TLDblocks=="EAS.hg19"){
    LDB=ASN.hg19
  }else if(data$TLDblocks=="EAS.hg38"){
    LDB=ASN.hg38
  }else if(data$TLDblocks=="AMR.hg19"){
    LDB=AFR.hg19
  }else if(data$TLDblocks=="AMR.hg38"){
    LDB=AFR.hg38
  }else if(data$TLDblocks=="SAS.hg19"){
    LDB=ASN.hg19
  }else if(data$TLDblocks=="SAS.hg38"){
    LDB=ASN.hg38
  }else{
    stop(paste(data$TLDblocks,"not found for LD blocks, please correct"))
  }
  ...
  if(is.na(sum(lambda))){
    Lmd=MaxLambda(XtY, tilde_beta=t(t(Beta_prior)), XG$X, t(t(K1)), m=t(t(rep(1,p))), blk, K0, tau=tau,
                  eta=0, alpha=alpha, eps=eps,max_iter=max_iter)
  ...
  for(eta in eta_vec){
    ...
    AIC=cbind(AIC,2*(rst_BRIGHTs$dev2*data$TN+colSums(as.matrix(rst_BRIGHTs$Beta)!=0)/(1+eta)))
    out[[as.character(eta)]]=rst_BRIGHTs
  }
  dinom=dim(AIC)[1]
  ind=which.min(AIC)
  rw=ind%%dinom
